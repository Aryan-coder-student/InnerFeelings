...
router = APIRouter(prefix="/video_gen", tags=["generation"])
...
@router.get("/text-to-video/file")
def get_video():
    """
    Download the last generated video.
    """
    path = VIDEO_DIR / "video.mp4"
    if not path.exists():
        raise HTTPException(status_code=404, detail="Video not found. Generate one first.")
    return FileResponse(str(path), media_type="video/mp4", filename="video.mp4")
...
def get_image():
    ...
    if not path.exists():
        raise HTTPException(status_code=404, detail="Image not found. Generate one first.")
    return FileResponse(str(path), media_type="image/png", filename="image.png")
